...
import { takeOverConversation } from '@/lib/conversation-service';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ caseId: string }> }) {
  ...
  if (!(await rateLimit(`conversations:takeover:${tenant.ctx.userId}`, 20, 60_000))) {
  ...
  try {
    ...
    if (!result.ok) {
      ...
      return NextResponse.json(
        { error: RESULT_MESSAGE[result.error_code] ?? 'Não foi possível assumir a conversa.', code: result.error_code },
        { status }
      );
    }
    return NextResponse.json(result.conversation);
  } catch (error) {
    return serverError('conversation takeover exception', error);
  }
}
