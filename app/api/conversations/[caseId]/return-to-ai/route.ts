...
import { returnConversationToAI } from '@/lib/conversation-service';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ caseId: string }> }) {
  ...
  if (!(await rateLimit(`conversations:return:${tenant.ctx.userId}`, 20, 60_000))) {
  ...
  try {
    ...
    if (!result.ok) {
      ...
      return NextResponse.json(
        { error: RESULT_MESSAGE[result.error_code] ?? 'Não foi possível devolver a conversa para a IA.', code: result.error_code },
        { status }
      );
    }
    return NextResponse.json(result.conversation);
  } catch (error) {
    return serverError('conversation return-to-ai exception', error);
  }
}
...
const RESULT_MESSAGE: Record<string, string> = {
  ...
  FORBIDDEN: 'Permissão insuficiente para realizar esta ação.',
  VERSION_CONFLICT: 'A conversa foi alterada por outro operador. Atualize antes de tentar novamente.',
  INVALID_STATE: 'Estado da conversa não permite esta ação.',
  INVALID_OPERATOR: 'Operador destinatário inválido.',
